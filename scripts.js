...
const firebaseConfig = {
  apiKey: "AIzaSyAzgd2Zt-giAcns0sh1xzTP506vhW7AiDo",
...
const instabilidadeMap = {
    '2025-01-31': 'Nº 102/2025 - Instabilidade do Projudi', '2025-04-04': 'Nº 207/2025 - Instabilidade do Projudi', '2025-05-23': 'Nº 279/2025 - Instabilidade do Projudi', '2025-05-29': 'Nº 288/2025 - Instabilidade do Projudi','2025-09-02': 'Nº 486/2025 - Instabilidade do Projudi',
};
...
const ConsultaAssistidaPJE = ({ numeroProcesso, setNumeroProcesso }) => {
    ...
    const logDjenUsage = () => {
        if (db && user) {
            ...
            }).catch(err => console.error("Erro ao registar uso da consulta DJEN:", err));
        }
    };
    ...
    try {
        ...
        if (tipoPrazo === 'civel') {
            ...
            const calcularPrazoCrime = (dataIntimacao, considerarDecretos) => {
                // Para prazos penais (dias corridos), adicionamos o número de dias ao início do prazo.
                // O prazo começa a contar no dia seguinte ao da intimação (exclui-se o dia do começo e inclui-se o do vencimento - Art. 798, § 1º, CPP).
                // Para somar N dias a uma data, efetivamente somamos N-1 dias para encontrar o dia final, pois o primeiro dia da contagem já é "dia 1".
                // Ex: Início 10/01, prazo 5 dias. Contagem: 11, 12, 13, 14, 15. O dia final é 15/01. Data(10) + 5 = Data(15).
                                ...
                                {resultado.decretoImpactou && (
                                    <div className="border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-700 md:pl-4 pt-4 md:pt-0">
                                       <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 text-center mb-2">Cenário 2: Com Decreto</h3>
                                        ...
                                        {resultado.comDecreto.diasNaoUteis.filter(d => d.tipo === 'decreto' || d.tipo === 'instabilidade').length > 0 && (
                                            <div className="mt-4 text-left border-t border-slate-300 dark:border-slate-600 pt-2"><p className="text-xs font-semibold text-orange-600 dark:text-orange-400 mb-1">Suspensões que influenciaram o prazo e precisam ser comprovadas:</p><ul className="text-xs space-y-1"><GroupedDiasNaoUteis dias={resultado.comDecreto.diasNaoUteis.filter(d => d.tipo === 'decreto' || d.tipo === 'instabilidade')} /></ul></div>
                                        )}
                                    </div>
                                )}
                        </div>
...
const LoginPage = () => {
    ...
    const handlePasswordReset = async (e) => {
        ...
        const finalEmail = `${email}@tjpr.jus.br`;
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                // Envia o e-mail de verificação primeiro para garantir que o usuário possa validar a conta.
                await userCredential.user.sendEmailVerification();
                // Em seguida, salva os dados do usuário no Firestore.
                await db.collection('users').doc(userCredential.user.uid).set({
                    email: finalEmail, isAdmin: false, displayName: displayName.trim() 
                });
                setMessage("Conta criada! Enviámos um link de verificação para o seu e-mail.");
            }
        } catch (err) {
            if (!isLogin) auth.signOut(); // Garante que o usuário seja deslogado se o cadastro falhar.
            switch (err.code) {
...
const VerifyEmailPage = () => {
    ...
    const [message, setMessage] = useState('');
    const [error, setError] = useState('');
    const [cooldown, setCooldown] = useState(60); // Inicia com cooldown de 60s para o primeiro envio
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            setShowButton(false); // Oculta o botão e mostra o timer novamente
            setCooldown(60); // Inicia cooldown de 60 segundos
        } catch (err) {
            if (err.code === 'auth/too-many-requests') {
                setError('Muitas tentativas. Por favor, aguarde um momento antes de tentar novamente.');
            } else {
                setError('Falha ao reenviar o e-mail. Tente novamente mais tarde.');
            }
            setShowButton(false); // Oculta o botão mesmo em caso de erro
            setCooldown(60); // Inicia cooldown mesmo em caso de erro para evitar spam
        }
    };
         ...
         <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-200 dark:from-slate-800 dark:to-slate-900 relative">
            <div className="w-full max-w-md p-8 space-y-6 bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl rounded-2xl shadow-lg text-center">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                    A entrega do e-mail pode levar alguns minutos.
                </p>
                <div className="space-y-4">
                    {showButton ? (
                        <button onClick={handleResend} className="w-full bg-gradient-to-br from-indigo-500 to-indigo-600 text-white font-semibold py-3 rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 shadow-md">
                            Reenviar E-mail
                        </button>
                    ) : (
                        <div className="text-center py-3 text-slate-500 dark:text-slate-400">Você poderá reenviar o e-mail em {cooldown}s</div>
                    )}
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const usersMap = usersSnapshot.docs.reduce((acc, doc) => ({ ...acc, [doc.id]: doc.data().displayName }), {});
         ...
         <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            {!hasSearched ? (
                ...
                <div className="text-center p-8 bg-slate-100/70 dark:bg-slate-900/50 rounded-lg">
                    <p className="text-slate-500 dark:text-slate-400">Nenhum resultado encontrado para os filtros selecionados.</p>
                </div>
            ) : (
                <>
...
const CalculatorApp = () => {
    ...
    const { isAdmin, user, userData, openCalendario } = useAuth();
    const [showNameAlert, setShowNameAlert] = useState(false);

    useEffect(() => {
        // Mostra o alerta se o usuário não tiver um nome de exibição e o alerta ainda não foi dispensado nesta sessão.
        if (user && user.emailVerified && !userData?.displayName && !sessionStorage.getItem('nameAlertDismissed')) {
            setShowNameAlert(true);
        } else {
            setShowNameAlert(false); // Garante que o alerta seja removido se o nome for preenchido
        }
    }, [user, userData]);

    const dismissNameAlert = () => {
        setShowNameAlert(false);
        sessionStorage.setItem('nameAlertDismissed', 'true');
    };
        ...
        <nav className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border-b border-slate-200/50 dark:border-slate-700/50">
            {showNameAlert && (
                <div className="bg-yellow-100 dark:bg-yellow-900/30 border-b border-yellow-200 dark:border-yellow-800/50 p-3">
                    <div className="container mx-auto flex items-center justify-center gap-4 text-sm text-yellow-800 dark:text-yellow-200 text-center">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>
                        <span><b>Atenção:</b> Seu nome de exibição não está definido. Para alterar, clique no menu (três pontos) no canto superior direito &rarr; Perfil.</span>
                        <button onClick={openProfileToEdit} className="ml-2 font-bold bg-yellow-200 dark:bg-yellow-500/50 text-yellow-900 dark:text-yellow-100 px-3 py-1 rounded-md hover:bg-yellow-300 transition-colors whitespace-nowrap">Alterar Agora</button>
                        <button onClick={dismissNameAlert} className="ml-auto p-1 rounded-full hover:bg-yellow-200/50 dark:hover:bg-yellow-800/50"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button>
                    </div>
                </div>
            )}
            <div className="container mx-auto px-4">
              <div className="flex justify-center items-center space-x-2 p-2">
                 ...
                 <button onClick={() => document.dispatchEvent(new CustomEvent('openCalendario'))} className="sm:hidden px-4 py-2 text-sm font-semibold rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
                    Calendário
                 </button>
